...
def main(base_path):
  ...
  try:
    ...
    #findspark.add_packages(["org.apache.spark:spark-streaming-kafka_2.11:1.6.3"])
    #findspark.init()
  ...
  def classify_prediction_requests(rdd):
    ...
    if final_predictions.count() > 0:
      final_predictions.rdd.map(lambda x: x.asDict()).saveToMongoDB("mongodb://localhost:27017/agile_data_science.flight_delay_classification_response")
